...
- Reads structure from CLI positional arg or config (npf.count/general: input/structure)
...
from .site_count import detect_sites_by_cn, TERRACE_TYPES, TERRACE_COUNT_KEYS
from .utils import NeighborGraph, build_neighbor_graph


def _resolve_input_path(args, cfg):
    """Pick the input file from CLI positional or config (npf.count/general)."""
...
def _default_out_path(input_file: str, prefer_csv: bool = True) -> str:
    ...
    stage = "after_relax" if "relaxed" in base_name else "before_relax"
    suffix = "csv" if prefer_csv else "txt"
    return os.path.join(base_dir, f"site_count_{stage}.{suffix}")
...
def write_single_csv(
    ...
    *,
    ...
    with open(out_path, "w", newline="") as f:
        ...
        for k in ("atop", "bridge", "hollow3", "hollow4_square", "hollow4_rect", "kink", "B5", "surface_atoms", *TERRACE_COUNT_KEYS):
            if k in counts:
        ...
        for typ in ("bridge", "hollow3", "hollow4_square", "hollow4_rect", "kink", "B5", *TERRACE_COUNT_KEYS):
            for item in sites.get(typ, []):
                w.writerow([typ, "", "", "", "", "", "", "", "", "", _fmt_indices(item)])


def count_surface_atoms(
    *,
    atoms,
    mode: str,
    cutoff,
    ...
    """
    Compute only the CN array and surface-atom count.

    This is the narrow version of the original workflow when the caller only
    needs the surface atom total and not the full site inventory.
    """
    ...
    return {
        "atoms": len(atoms),
        "surface_atoms": int(np.sum(surface_mask)),
        "surface_indices": np.where(surface_mask)[0].tolist(),
        "cutoff_info": cutoff_info,
        "surface_threshold": int(surface_threshold),
        "cn_arr": cn_arr,
        "cn_hist": [(int(c), int(h)) for c, h in zip(((edges_h[:-1] + edges_h[1:]) * 0.5).astype(int), hist)],
    }
...
def build_parser():
    ...
    ap.add_argument("--config", help="Config INI (default: ./npf.count.in or ~/.np_sites/npf.count.in)")
...
def main(argv=None):
    ...
    if not input_file:
        print("ERROR: no input structure (provide STRUCTURE or set input= / structure= in npf.count.in)", file=sys.stderr)
        sys.exit(2)
    ...
    print("\nTerrace interiors (excluding patch-boundary atoms):")
    for kind in TERRACE_TYPES:
        print(f"  {kind}: {_n(kind + '_interior_atoms')} atoms; "
              f"{_n(kind + '_interior_sites')} all-interior hollow sites")
    print(f"\nWrote: {out_path}")
